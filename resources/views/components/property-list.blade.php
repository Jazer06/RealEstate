...
.card-text {
    margin: 0;
    max-height: 60px; /* Ограничение высоты текста с типом и адресом */
    overflow: hidden;
...
.card-description {
    margin: 0;
    max-height: 80px; /* Ограничение высоты описания */
    overflow: hidden;
    text-overflow: ellipsis;
    display: -webkit-box;
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                ...
                                <p class="card-description">{{ $property->description ?? 'Нет описания' }}</p>                            
                            </div>
                            <div class="d-flex justify-content-end pr-30 mt-45">
                                @auth
                                    <form action="{{ route('favorites.toggle', $property->id) }}" method="POST" onclick="event.stopPropagation();">
                                        @csrf
                                        <button type="submit" class="p-1 favorite-icon {{ auth()->user()->favorites->contains($property->id) ? 'favorite-added' : '' }}">
